...
def plot_images_and_actions(dataset_name, curr_viz_obs_image, curr_viz_goal_image, curr_viz_pred_actions, curr_viz_actions, min_idx, loss):
    spacing = data_config[dataset_name]['metric_waypoint_spacing']
    print(f"[DEBUG] Plotting using dataset: {dataset_name}, Spacing: {spacing}")
    ...
    plot_array = np.frombuffer(buf, dtype='uint8')
    w, h = canvas.get_width_height()
    plot_array = plot_array.reshape(h, w, 4)[..., :3]
    plt.close(fig)
    return plot_array
...
def get_delta_np(actions):
    ...
    for i in range(delta.shape[0]):
        delta[i,:2]=to_local_coords(ex_actions[i+1,:2],ex_actions[i,:2],ex_actions[i,2])
    delta[:,2] = delta[:,2] - 2 * np.pi * np.floor((delta[:,2] + np.pi) / (2 * np.pi))    
    return delta
def to_local_coords(
